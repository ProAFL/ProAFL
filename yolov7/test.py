...
def collect_sample_train_indicator(
        model, # 从训练脚本传递过来的
        data,
        dataloader,
        compute_loss,
    ...
    for batch_i, (img, targets, paths, shapes) in enumerate(dataloader):
        ...
        with torch.no_grad():
            ...
            loss_box,loss_obj,loss_cls,loss = compute_loss([x.float() for x in train_out], targets)[1][:4]  # box, obj, cls
            item["loss_box"] = loss_box.item()
            item["loss_obj"] = loss_obj.item()
            item["loss_cls"] = loss_cls.item()
            item["loss"] = loss.item()
            # Run NMS
            targets[:, 2:] *= torch.Tensor([width, height, width, height]).to(device)  # to pixels
            lb = []  # for autolabelling
            out = non_max_suppression(out, conf_thres=0.25, iou_thres=0.45, labels=lb, multi_label=True)
        # Statistics per image, si当前batch中图片的索引
        conf_list = []
        for si, pred in enumerate(out):
            ...
            path = Path(paths[si])
            item["image_path"] = paths[si]
            if len(pred) == 0:
                item["conf_avg"] = 0
                item["box_count_dif"] = len(labels)
                continue
            predn = pred.clone()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--weights', nargs='+', type=str, 
                        # default=f'trained_models/{dataset_name.lower()}/datactive_best.pt'
